...
while getopts s:g:n:d:p:r:h OPTION; do
    case "$OPTION" in
        s) sweep_id=${OPTARG};;
        g) gpu_id=${OPTARG};;
...
CUDA_VISIBLE_DEVICES=$gpu_id torchrun --nproc-per-node $nproc_per_node --master-port $port ./cifar10/main_cifar.py --data_set $data_name --data_root /export/Lab-Xue/projects/$data_name/data --check_path /export/Lab-Xue/projects/$data_name/checkpoints --model_path /export/Lab-Xue/projects/$data_name/models --log_path /export/Lab-Xue/projects/$data_name/logs --results_path /export/Lab-Xue/projects/$data_name/results --num_epochs 150 --batch_size 128 --device cuda --global_lr 1e-3 --clip_grad_norm 1.0 --weight_decay 0.0 --use_amp --iters_to_accumulate 1 --project cifar --num_workers 8 --scheduler_type OneCycleLR --scheduler.ReduceLROnPlateau.patience 2 --scheduler.ReduceLROnPlateau.cooldown 2 --scheduler.ReduceLROnPlateau.min_lr 1e-7 --scheduler.ReduceLROnPlateau.factor 0.9 --scheduler.StepLR.step_size 5 --scheduler.StepLR.gamma 0.8 --backbone_hrnet.num_resolution_levels 3 --backbone_hrnet.use_interpolation 1 --backbone_unet.num_resolution_levels 3 --backbone_unet.use_unet_attention 1 --backbone_unet.use_interpolation 1 --backbone_unet.with_conv 1 --backbone_LLM.num_stages 3 --backbone_LLM.add_skip_connections 1 --backbone_small_unet.channels 16 32 64 --backbone_small_unet.block_str T1L1G1 T1L1G1 T1L1G1 --run_name cifar-sweep --run_notes cifar-sweep --backbone hrnet --a_type conv --cell_type sequential --cosine_att 1 --att_with_relative_postion_bias 1 --backbone_hrnet.C 64 --backbone_unet.C 64 --backbone_LLM.C 64 --block_dense_connection 1 --norm_mode batch2d --mixer_type conv --shuffle_in_window 0 --mixer_kernel_size 5 --mixer_padding 2 --mixer_stride 1 --normalize_Q_K --backbone_hrnet.block_str T1L1G1 T1L1G1 T1L1G1 --ddp --ratio $tra_ratio 100 0 --sweep_id $sweep_id