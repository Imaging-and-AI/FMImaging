        ...
        with tqdm(total=len(keys[i]), bar_format=get_bar_format()) as pbar:
            for n, key in enumerate(keys[i]):
                if num_loaded < max_load:
                    ...
                    images.append([np.array(h5file[i][key+"/noisy_im"]), np.array(h5file[i][key+"/clean_im"]), i])
                    num_loaded += 1
                else:
                    images.append([key+"/noisy_im", key+"/clean_im", i])
