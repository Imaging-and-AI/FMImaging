...
def custom_SWIN(config, input_feature_channels):
    """
    NOTE: THIS ARCHITECTURE IS NOT WRITTEN FOR MORE THAN ONE INPUT TASK
    Wrapper function to set up and return SWIN model.
    @args:
    ...
    elif config.SWIN.size=='custom':
        embed_dim = config.SWIN.embed_dim
        depths =  config.SWIN.depths
        num_heads =  config.SWIN.num_heads
    else:
        raise ValueError(f"Unknown model size {config.SWIN.size} specified in config.")

    if config.time[0]==1:
        spatial_dims = 2
        if len(config.SWIN.patch_size)==3: 
            mod_patch_size = config.SWIN.patch_size[1:]
            mod_window_size = config.SWIN.window_size[1:]
        else: 
            mod_patch_size = config.SWIN.patch_size
            mod_window_size = config.SWIN.window_size
    else:
        spatial_dims = 3
        mod_patch_size = config.SWIN.patch_size
        mod_window_size = config.SWIN.window_size
    ...
    model = SwinTransformer_with_hyena(use_hyena=config.SWIN.use_hyena,
                in_chans=input_feature_channels[-1],
                embed_dim=embed_dim,
                window_size=mod_window_size,
...
class ModPatchEmbed(nn.Module):
    """
    Code adapted from Monai
    ...
    def __init__(
        ...
        Args:
            ...
            use_mae: using masked auto encoding.
        """
        ...
        if self.use_mae:
            if spatial_dims==2: self.mask_token = nn.Parameter(torch.zeros(1, embed_dim, 1, 1))
            else: self.mask_token = nn.Parameter(torch.zeros(1, embed_dim, 1, 1, 1))
    ...
    def forward(self, x):
        ...
        if len(x_shape) == 5:
            ...
            if w % self.patch_size[2] != 0:
                x = F.pad(x, (0, self.patch_size[2] - w % self.patch_size[2]), mode='reflect')
            if h % self.patch_size[1] != 0:
                x = F.pad(x, (0, 0, 0, self.patch_size[1] - h % self.patch_size[1]), mode='reflect')
            if d % self.patch_size[0] != 0:
                x = F.pad(x, (0, 0, 0, 0, 0, self.patch_size[0] - d % self.patch_size[0]), mode='reflect')

        elif len(x_shape) == 4:
            _, _, h, w = x_shape
            if w % self.patch_size[1] != 0:
                x = F.pad(x, (0, self.patch_size[1] - w % self.patch_size[1]), mode='reflect')
            if h % self.patch_size[0] != 0:
                x = F.pad(x, (0, 0, 0, self.patch_size[0] - h % self.patch_size[0]), mode='reflect')
        x_og = x.clone()
...
class SwinTransformer_with_hyena(nn.Module):
    """
    ...
    def __init__(
        ...
        self.use_mae = use_mae
        self.patch_embed = ModPatchEmbed(
    ...
    def forward(self, x, normalize=True):
        ...
        hidden_states_out += [x0_out]
        if self.use_v2:
            x0 = self.layers1c[0](x0.contiguous())
        x1 = self.layers1[0](x0.contiguous())
        x1_out = self.proj_out(x1, normalize)
        hidden_states_out += [x1_out]
        if self.use_v2:
            x1 = self.layers2c[0](x1.contiguous())
        x2 = self.layers2[0](x1.contiguous())
        x2_out = self.proj_out(x2, normalize)
        hidden_states_out += [x2_out]
        if self.use_v2:
            x2 = self.layers3c[0](x2.contiguous())
        x3 = self.layers3[0](x2.contiguous())
        x3_out = self.proj_out(x3, normalize)
        hidden_states_out += [x3_out]
        if self.use_v2:
            x3 = self.layers4c[0](x3.contiguous())
        x4 = self.layers4[0](x3.contiguous())
        x4_out = self.proj_out(x4, normalize)
        hidden_states_out += [x4_out]
        if self.spatial_dims==2:
            hidden_states_out = [x.unsqueeze(2) for x in hidden_states_out]
