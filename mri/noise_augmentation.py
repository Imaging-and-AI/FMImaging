...
def generate_MR_correlated_noise(RO=192, E1=144, REP=1, 
                                 min_noise_level=3.0, 
                                 max_noise_level=7.0, 
                                 kspace_filter_sigma=[0.8, 1.0, 1.5, 2.0, 2.25],
                                 pf_filter_ratio=[1.0, 7/8, 6/8, 5/8],
    ...
    if(REP>1):
        nn = create_complex_noise(noise_sigma, (RO, E1, REP))
    else:
        nn = create_complex_noise(noise_sigma, (RO, E1))
    
    if(verbose is True):
        print("--" * 40)
        print(f"noise sigma is {noise_sigma}")
        print("noise, real, std is ", np.mean(np.std(np.real(nn), axis=1)))
        print("noise, imag, std is ", np.mean(np.std(np.imag(nn), axis=1)))
    ...
    # no need to apply snr scaling here
    ...
    # apply kspace filter
    kspace_filter_sigma = kspace_filter_sigma[rng.integers(0, len(kspace_filter_sigma))]
    ...
    nn = ifft2c(apply_kspace_filter_2D(fft2c(nn), fRO_used, fE1_used))
        
    if(verbose is True):
        print("--" * 20)
        print(f"kspace_filter_sigma is {kspace_filter_sigma}")
        
    if(verbose is True):
        print("--" * 20)
        std_r = np.mean(np.std(np.real(nn), axis=1))
        std_i = np.mean(np.std(np.imag(nn), axis=1))
