...
def create_cmd_run(cmd_run, 
    ...
    run_str = f"{a_type}-{cell_type}-{norm_mode}-{optim}-{scheduler_type}-C-{c}-block_dense-{block_dense_connection}-qknorm-{q_k_norm}-cosine_att-{cosine_att}-att_with_relative_postion_bias-{att_with_relative_postion_bias}-block_str-{'_'.join(bs)}"
    ...
    cmd_run.extend([
        ...
        "--optim", f"{optim}",
        "--scheduler_type", f"{scheduler_type}"
    ])
