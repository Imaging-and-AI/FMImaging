...
def transform_f(x):
    ...
    @rets:
        - x (torch.Tensor): 4D torch tensor [C,T,H,W], T=1
    """
...
class cifar_dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, split):
        ...
        self.config = config
        self.data_loc = config.data_dir[0]
        self.height = config.height
        self.width = config.width
        self.split = split

        if self.split=='train':
            transform = transforms.Compose([transforms.Resize((self.height[0],self.width[0])),  #resises the image so it can be perfect for our model.
            ...
            transform = transforms.Compose([transforms.Resize((self.height[0],self.width[0])),
                                                transforms.ToTensor(),
