...
def load_images_from_h5file(h5files, keys, clean_name="SOC_AiCE", max_load=100000):
    """
    ...
    @args:
        ...
        - clean_name (string): the name of image taken as clean key
        ...
        with tqdm(total=len(keys[i])) as pbar:
            for n, key_1 in enumerate(keys[i]):
                ...
                for key_2 in h5files[i][key_1]:
                    ...
                    if key_2.lower() == clean_name.lower():
...
class CTDatasetTrain():
    ...
    """
    def __init__(self, h5file, keys, clean_name="SOC_AiCE", max_load=10000,
        ...
        self.images = load_images_from_h5file(h5file, keys, clean_name, max_load=max_load)
...
class CTDatasetTest():
    ...
    def load_one_sample(self, i, noisy_im_idx):
        ...
        noisy_im = noisy_im[np.newaxis,:min_t,:,:]
        clean_im = clean_im[np.newaxis,:min_t,:,:]
...
def load_ct_data_all(config):
    ...
    @rets:
        - train_set, val_set, test_set (CT dataloader list): the datasets
    """
    ...
    if c.train_only:
        ...
        test_set = [CTDatasetTest(h5file=h5files, keys=test_keys,
                                  clean_name=c.clean_name, max_load=c.max_load)]
    else:
