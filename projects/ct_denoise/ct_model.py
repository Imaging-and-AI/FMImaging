...
- STCNNT_CT: the pre-backbone-post model with a simple pre and post module
- CT_double_net: a hrnet or mixed_unet backbone + a hrnet or mixed_unet post
"""
...
# input to the CT models are channel-first, [B, C, T, H, W]
# output from the CT models are channel-first, [B, C, T, H, W]
# inside the model, permutes are used as less as possible

# -------------------------------------------------------------------------------------------------

def ct_ModelManager(config):
    """
    Model selector based on cli args
    """
    config_copy = copy.deepcopy(config)
    if config.model_type == "STCNNT_CT":
        model = STCNNT_CT(config=config_copy)
    elif config.model_type == "CT_double_net":
        model = CT_double_net(config=config_copy)
    else:
        raise NotImplementedError(f"CT model not implemented: {config.model_type}")
...
class STCNNT_CT(ModelManager):
    ...
    def __init__(self, config):
        ...
        config.height = config.ct_height[-1]
        config.width = config.ct_width[-1]
