"""
Training and evaluation loops for CT
"""
...
class CTTrainManager(TrainManager):
    """
    CT train manager
        - support CT double net
    """
    ...
    def _eval_model(self, rank, model_manager, data_sets, epoch, device, optim, sched, id, split, final_eval):
        ...
        cutout = (c.ct_time, c.ct_height[-1], c.ct_width[-1])
        overlap = (c.ct_time//2, c.ct_height[-1]//4, c.ct_width[-1]//4)
        ...
        with torch.inference_mode():
            with tqdm(total=total_iters, bar_format=get_bar_format()) as pbar:
                ...
                for idx in range(total_iters):
                    ...
                    if B > 1 and (W==c.ct_width[-1] or W==c.ct_width[0]):
                        output = model_manager(inputs)
                        ...
                        if not self.config.pad_time:
                            cutout_in = (T, c.ct_height[-1], c.ct_width[-1])
                            overlap_in = (0, c.ct_height[-1]//2, c.ct_width[-1]//2)
