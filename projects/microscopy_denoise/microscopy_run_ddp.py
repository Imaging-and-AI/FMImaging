...
Microscopy_DIR = Path(__file__).parents[0].resolve()
...
class microscopy_ddp_base(run_ddp_base):
    ...
    def create_cmd_run(self, cmd_run, config, 
        ...
        cmd_run.extend(["--micro_height"])
        cmd_run.extend([f"{h}" for h in config.micro_height])

        cmd_run.extend(["--micro_width"])
        cmd_run.extend([f"{w}" for w in config.micro_width])
    ...
    def arg_parser(self):
        ...
        parser.add_argument("--micro_height", nargs='+', type=int, default=[32, 64], help='heights of the training images')
        parser.add_argument("--micro_width", nargs='+', type=int, default=[32, 64], help='widths of the training images')

        parser.add_argument("--model_type", type=str, default="STCNNT_Microscopy", help="STCNNT_Microscopy or Microscopy_double_net")
...
def main():
    ...
    ddp_run = microscopy_ddp_base(project="microscopy", script_to_run=str(REPO_DIR)+"/projects/microscopy_denoise/microscopy_run.py")
    ddp_run.run()
