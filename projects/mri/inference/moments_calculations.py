...
def get_eigvecs(model: torch.nn.Module,
    ...
    mask = mask.to(device)
    with torch.no_grad():
        im_mmse = _forward_directional(model, input, device, 0, 0, double_precision).squeeze(0)
    ...
    for i in tqdm(range(iters)):
        if compare_backprop:
            eigvecs, jacprod_norm, eigvecs_backprop, jacobprod_backprop_norm, (apprx_time, bp_time) = _power_iteration(
                model, input, nim, n_ev, device, b_masked_mmse, eigvecs, double_precision, mask, compare_backprop=True, c=c)
            timing.append((apprx_time, bp_time))
        else:
            eigvecs, jacprod_norm = _power_iteration(model, input, nim, n_ev, device, b_masked_mmse, eigvecs, double_precision, mask, compare_backprop=False, c=c)
        _, tmp = (jacprod_norm / c * (sigma**2)).reshape(n_ev, ).sort(descending=True, stable=True)
        eigvecs = eigvecs[tmp, ...]
        # next = eigvecs.clone()
        # next = eigvecs[tmp, ...].clone()
        loss.append((prev - eigvecs).norm().item()/ n_ev)
    ...
    print(loss)
    with torch.no_grad():
        unmaksed_out = _forward_directional(model, input, device, eigvecs, c, double_precision)
        out = unmaksed_out * mask
        Ab = out - b_masked_mmse
    jacprod_norm = Ab[:, mask.to(torch.bool)].norm(dim=1)
    #eigvals = (jacprod_norm / c * (sigma**2)).reshape(n_ev, )
    eigvals = (jacprod_norm * (sigma**2)).reshape(n_ev, )
    eigvecs /= c
    if compare_backprop:
...
def calc_moments(model: torch.nn.Module,
    ...
    with torch.no_grad():
        ...
        second_moments, third_moments, fourth_moments, second_moments_per_pixel = _calc_second_third_and_fourth_moments(
            model, input, nim, n_ev, device, var_c, skew_c, kurt_c, b_masked_mmse, eigvecs, sigma,
        ...
        # more_fourth_moments = _calc_fourth_moments(model, input, nim, n_ev, device, 10*kurt_c, b_masked_mmse, eigvecs,
    ...
    return Moments(vmu1=first_moments.cpu().numpy().astype(np.float64),
                   ...
                   vmu2_per_pixel=second_moments_per_pixel.cpu().numpy().astype(np.float64)
                   )
...
def _forward_directional(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        if isinstance(eigvecs, torch.Tensor):
            C = eigvecs.shape[1]
            input = patch.repeat(eigvecs.shape[0], 1, 1, 1, 1)
            input[:,0:C] = patch[:,0:C] + amount * eigvecs
        else:
...
def _power_iteration(model: torch.nn.Module, 
                     input: torch.Tensor, 
                     nim: torch.Tensor,
    ...
    if compare_backprop:
        ...
        if n_ev > 1:
            ...
            if len(nim.shape) == 5:
                permute_arg = (1, 2, 3, 4, 0)
            elif len(nim.shape) == 4:
                permute_arg = (1, 2, 3, 0)
...
def _calc_fourth_moments(model: torch.nn.Module,
    ...
    first_term = _forward_directional(model, input, device, eigvecs, 1.5 * c, double_precision) * mask
    second_term = _forward_directional(model, input, device, eigvecs, 0.5 * c, double_precision) * mask
    # third_term = b_masked_mmse
    third_term = _forward_directional(model, input, device, eigvecs, -0.5 * c, double_precision) * mask
    fourth_term = _forward_directional(model, input, device, eigvecs, -1.5 * c, double_precision) * mask
...
def _calc_third_moments(model: torch.nn.Module, input: torch.Tensor, nim: torch.Tensor, n_ev: int, device: torch.device,
                        c: float, bmmse: torch.Tensor, eigvecs: torch.Tensor, sigma: float,
                        mask: torch.Tensor, double_precision: bool):
    mid_term = bmmse
    left_term = _forward_directional(model, input, device, eigvecs, -c, double_precision) * mask
    right_term = _forward_directional(model, input, device, eigvecs, c, double_precision) * mask
...
def _calc_third_and_fourth_moments(model: torch.nn.Module, 
                                   input: torch.Tensor, 
                                   nim: torch.Tensor,
                                   n_ev: int, device: torch.device,
    ...
    if use_poly:
        third_moments, fourth_moments = _calc_third_and_fourth_moments_by_polyfit(model, input, nim, n_ev, device,
                                                                                  second_moments, eigvecs,
                                                                                  sigma, mask, double_precision,
                                                                                  poly_deg, poly_bound, poly_pts)
    elif kurt_c == skew_c:
        c = kurt_c
        first_term = _forward_directional(model, input, device, eigvecs, 2 * c, double_precision) * mask
        second_term = _forward_directional(model, input, device, eigvecs, c, double_precision) * mask
        third_term = b_masked_mmse
        fourth_term = _forward_directional(model, input, device, eigvecs, -c, double_precision) * mask
        ...
        third_moments = _calc_third_moments(model, input, nim, n_ev, device, skew_c, b_masked_mmse, eigvecs, sigma, mask,
                                            double_precision)
        fourth_moments = _calc_fourth_moments(model, input, nim, n_ev, device, kurt_c, b_masked_mmse, eigvecs, second_moments,
                                              sigma, mask, double_precision)
    return third_moments, fourth_moments


def _calc_second_third_and_fourth_moments(model: torch.nn.Module, 
                                          input: torch.Tensor, 
                                          nim: torch.Tensor,
                                          n_ev: int, device: torch.device,
    ...
    second_term = _forward_directional(model, input, device, eigvecs, 0.5 * var_c, double_precision) * mask
    third_term = _forward_directional(model, input, device, eigvecs, -0.5 * var_c, double_precision) * mask
    second_moments_per_pixel = (1 / var_c) * (sigma ** 2) * ( eigvecs.reshape(n_ev, -1) * (second_term - third_term).reshape(n_ev, -1) )
    second_moments = second_moments_per_pixel.sum(axis=1)

    second_moments_per_pixel = torch.reshape(second_moments_per_pixel, [n_ev, *nim.shape[1:]])

    # second_moments = eigvals
    print(f'second_moments: {second_moments}')

    third_moments = second_moments
    fourth_moments = second_moments
    ...
    #                                         double_precision)
    #     if kurt_c == var_c:
    #         first_term = _forward_directional(model, input, device, eigvecs, 1.5 * kurt_c, double_precision) * mask
    #         fourth_term = _forward_directional(model, input, device, eigvecs, -1.5 * kurt_c, double_precision) * mask

    #         deriv_approx = (1/(kurt_c**3)) * (first_term - 3 * second_term + 3 * third_term - fourth_term)
    #         fourth_moments = (sigma ** 6) * (eigvecs.reshape(n_ev, -1) * deriv_approx.reshape(n_ev, -1)).sum(axis=1) + \
    #             3 * (second_moments ** 2)
    #     else:
    #         fourth_moments = _calc_fourth_moments(model, input, nim, n_ev, device, kurt_c, b_masked_mmse, eigvecs,
    #                                               second_moments, sigma, mask, double_precision)

    return second_moments, third_moments, fourth_moments, second_moments_per_pixel
...
def _calc_third_and_fourth_moments_by_polyfit(model: torch.nn.Module,
    ...
    for i in tqdm(range(n_ev)):
        ...
        batched_inps = input[0].repeat(poly_pts, *[1]*(len(input.shape) - 1)).to(device) + batched_eigvs
        batched_outs = []
