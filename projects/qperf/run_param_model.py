...
from qperf_data import load_qperf_data
from qperf_loss import qperf_loss
from qperf_model import QPerfBTEXModel, QPerfParamsModel
...
def main():
    ...
    loss_f = qperf_loss(config=config)
    ...
    assert config.model_btex_load_path is not None

    # -----------------------------------------------

    model = QPerfParamsModel(config=config_for_model, 
    ...
    model.post.load(config.model_btex_load_path)
    ...
    metric_manager = QPerfMetricManager(config=config)
